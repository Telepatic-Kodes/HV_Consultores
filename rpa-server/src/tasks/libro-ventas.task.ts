...
// Libro Ventas Task - Download sales records
...
export class LibroVentasTask extends BaseTask {
  ...
  async executeSteps(): Promise<Record<string, unknown>> {
    ...
    if (!periodo) {
      throw new Error('Período es requerido para descargar libro de ventas')
    }
    ...
    // Navigate to libro ventas
    await this.page.goto(SII_CONFIG.URLS.LIBRO_VENTAS, {
      waitUntil: 'networkidle',
      timeout: SII_CONFIG.TIMEOUTS.PAGE_LOAD,
    })

    await this.takeScreenshot('libro_ventas_page')
    ...
    if (downloadButton) {
      ...
      const filename = `libro_ventas_${periodo}.csv`

...
**Update layout.tsx:**
Ensure `src/app/layout.tsx` no longer imports any Supabase auth provider or session context. It should only wrap children in `ConvexClientProvider`. Remove any Supabase-related provider wrappers (SessionProvider, AuthProvider, etc.) if present.
  </action>
  <verify>
...
- `npm run dev` starts without module not found errors
  ...
  <done>
Supabase client files deleted. Middleware no longer references Supabase. Layout.tsx uses only ConvexClientProvider. Critical Supabase imports removed; any remaining edge cases documented in SUMMARY for Phase 2 cleanup.
  </done>
</task>
